...
* 提交作业的文件名需以`HW-1-Team-X.Rmd`和`HW-1-Team-X.pdf`(或者`HW-1-Team-X.html`),请将`X`替换为小组编号，如`HW-1-Team-A.Rmd`和`HW-1-Team-A.pdf`。(若R Markdownc出现无法`knit`为`pdf`情况，可以将使用`bookdown::html_document2:`，则会生成为`html`)
* 请显示每道题的R Code于pdf中，注重Code的整洁性和可读性，可参考[Google's R Style Guide](https://google.github.io/styleguide/Rguide.html)
* 本次作业所需的数据和R Packages已经提供。本次作业需要的数据可以通过以下命令获取(并已经自动下载到R中):
...
dim(region_map_shp); dim(gtd_region);dim(wdi);dim(polity); dim(acled_cnty);
dim(ideal_point_wide); dim(ucdp_cnty)
```


```{r echo=FALSE}
df <- data.frame(dataset = c("region_map_shp", "gtd_region", "wdi",
                             "polity", "acled_cnty", "ideal_point_wide", "ucdp_cnty"),
                 variable_num = c(17769,9586,319, 241, 107, 1664, 17),
...
stargazer(, header=FALSE, type='latex', title = "描述性统计结果",
                     digit.separator = "")
...
```{r terrmap, fig.height=3, echo=FALSE,fig.cap='东南亚恐怖袭击数量分布。'}
...
```{r, echo=FALSE, fig.height=5, fig.cap='东南亚各国国民生产总值增长率变化（1990-2018）。'}
...
```{r, include=FALSE}
ggplot(acled_cnty, aes(year, events, group = event_type), linetype= event_type) +
  geom_point(colour = "pink", size = 2)+
...
```{r q10, eval=FALSE, fig.cap='东南亚各国冲突数量（1946-2019）。'}
# 此题的部分代码如下
ggplot(, aes(x = , y = , fill = )) + 
...
```{r, echo=FALSE, fig.cap='东南亚各国冲突数量（1946-2019）。'}
